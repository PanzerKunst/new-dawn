...
CB.Services.Browser = {
  ...
  cssRules: function() {
    if (CB.Services.Browser.allCssRules) {
      return CB.Services.Browser.allCssRules;
    }

    CB.Services.Browser.allCssRules = {};
    ...
    for (let i = 0; i < styleSheets.length; i++) {
      ...
      if (styleSheetRules) {
        for (let j = 0; j < styleSheetRules.length; j++) {
          ...
          CB.Services.Browser.allCssRules[rule.selectorText] = rule.style;
        }
      }
    }
  ...
  getCssRule: function(selector, property) {
    return CB.Services.Browser.cssRules()[selector].getPropertyValue(property);
  },
